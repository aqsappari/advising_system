...
<body class="bg-gray-300">
    ...
    <div class="flex justify-center">
        <div class="w-full md:w-[80vw] p-5 pr-7 bg-gray-100 shadow rounded-b mx-auto">
            <div class="px-4 pb-4 flex justify-between">
                <a href="<?= $_SESSION['location'] ?? '/' ?>" data-tooltip-target="go-back"
                    data-tooltip-placement="right"
                    class="my-auto px-2 me-2 text-2xl font-medium bg-white text-purple-600 focus:outline-none bg-transparent rounded shadow hover:bg-purple-300 hover:text-white focus:z-10 focus:ring-4 focus:ring-purple-300">
                    < </a>
            ...
            <form method="post" action="/action/add" class="w-full" onkeydown="return event.key != 'Enter';">
                ...
                <div class="border border-gray-100 bg-white shadow rounded my-3 mb-9 mx-1 p-5 relative w-full">
                    <span
                    ...
                    <div class="mb-6 grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="tel" id="year" name="year"
                                ...
                                value="<?= $curriculum['effective_from_ay'] ?>" required />
                    ...
                    <div class="mb-6 grid gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="college" name="college"
                                class="text-gray-900 text-sm focus:ring-blue-500 focus:outline-none block w-full p-2.5 bg-transparent border-b border-b-gray-300"
                                placeholder="20XX - 20XX+1" pattern="20[0-9]{2} - 20[0-9]{2}"
                                value="<?= $curriculum['college'] ?>" required />
                            <div id="errcCollege" class="text-right text-xs text-gray-500" hidden>Required</div>
                        </div>
                        <div>
                            <label for="dept" class="block mb-2 text-sm font-medium text-gray-900">
                                Department
                            </label>
                    ...
                    <div id="yearDivisor" class="w-full grid md:grid-cols-2 gap-4">
                        ...
                        foreach ($years as $year) {
                            foreach ($semesters as $semester) {
                                $elec_exist = false;
                                $yearDivisor = $year['subject_year'];


                                $YEARNAME = "";

                                switch (strval($yearDivisor)[sizeof($year) - 1]) {
                                    case "1":
                                        $YEARNAME =
                                            sizeof($year) > 1 && strval($yearDivisor)[0] === "1"
                                            ? strval($yearDivisor) . "th Year"
                                            : strval($yearDivisor) . "st Year";
                                        break;
                                    case "2":
                                        $YEARNAME =
                                            strval($yearDivisor) === "1"
                                            ? strval($yearDivisor) . "th Year"
                                            : strval($yearDivisor) . "nd Year";
                                        break;
                                    case "3":
                                        $YEARNAME =
                                            strval($yearDivisor) === "1"
                                            ? strval($yearDivisor) . "th Year"
                                            : strval($yearDivisor) . "rd Year";
                                        break;
                                    default:
                                        $YEARNAME = strval($yearDivisor) . "th Year";
                                        break;
                                }
                                ...
                                foreach ($subjects as $subject) {
                                    if ($subject['subject_year'] == $year['subject_year'] && $subject['subject_semester'] == $semester) {
                                ...
                                if (sizeof($subjectReturn) > 0) {
                                    ...
                                    <div class="p-2 shadow bg-gray-100/30">
                                        <div class="text-sm font-medium text-center"><?= "{$YEARNAME} - {$termDivisor}" ?></div>
                                            ...
                                            foreach ($subjectReturn as $subject) {
                                                ...
                                                if ($subject["subject_electives"] != "None") {
                                                    if (!$elec_exist) {
                                                        ...
                                                        <tr>
                                                            <td class="p-1 border border-black bg-yellow-300/10"></td>
                                                            <td class="text-xs text-center p-1 border border-black bg-yellow-300/10">
                                                                Elective Subject #<?= $elec_increment ?>
                                                            </td>
                                                            <td class="p-1 border border-black bg-yellow-300/10"></td>
                                                            <td class="p-1 border border-black bg-yellow-300/10"></td>
                                                            <td class="p-1 border border-black bg-yellow-300/10"></td>
                    ...
                    <div id="electiveDivisor" class="w-full grid md:grid-cols-2 gap-4">
                        ...
                        if (sizeof($electives) > 0) {
                            foreach ($electives as $elective) {
                                if ($elective['subject_electives'] != "None") {
                                    $subjectReturn = [];
                                    ...
                                    if (sizeof($subjectReturn) > 0) {
                                        ...
                                        <div class="p-2 shadow bg-gray-100/30">
                                            <div class="text-sm font-medium text-center"><?= "{$elective['subject_electives']}" ?></div>
                                            <table width="600" class="w-full">
                                                <tr class="bg-gray-200">
                                                    <th width="75" class="text-xs font-semibold text-center py-1 border border-black">
                                                        Code</th>
        ...
        <div class="relative p-4 w-login max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow ">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <a href="/curriculums/delete/<?= $curriculum['name'] ?>"
