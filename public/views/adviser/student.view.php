                ...
                <div class="mb-6 flex w-full justify-center bg-white shadow-lg rounded-sm border border-gray-200">
                    ...
                    <div class="flex-1 m-2 text-sm flex flex-col justify-around">
                        ...
                        <p class="mb-2 me-3 text-red-600">Year / Level: <span
                                class="text-nowrap font-semibold text-contrast"><?= $student['year_level'] ?></span></p>
                        <p class="mb-2 me-3 text-red-600">Status: <span
                                class="text-nowrap font-semibold <?= $student['status'] == "Not Yet Enrolled" ? "text-white bg-red-500 px-2 rounded-lg" : "text-contrast" ?>"><?= $student['status'] ?></span>
                        </p>
                                    ...
                                    if (sizeof($subjectReturn) > 0) { ?>
                                        <div type="submit" class="p-3 shadow">
                                            <div
                                                ...
                                                <div class="font-semibold"><?= "{$YEARNAME} - {$termDivisor}" ?></div>
                                            ...
                                            <table width="550" class="w-full">
                                                ...
                                                foreach ($subjectReturn as $subject) {
                                                    ...
                                                    <tr class="bg-yellow-100/10">
                                                        ...
                                                        <td class="text-xs text-center p-1 border border-black">
                                                            <?php
                                                            $subjectStatus = $db->query("SELECT status FROM `{$studentTable}` WHERE subject_code = '{$subject['subject_code']}'")->fetchAll();
                                                            $statusSubjects = [];
                                                            $statusShow = '';
                            ...
                            foreach ($years as $year) {
                                ...
                                <div id="YEAR_SEPARATOR" class="flex flex-col items-center my-6">
                                    <div class="flex justify-around gap-6 mb-6">
                                        <h1 class="text-2xl font-bold"> <?= $YEARNAME ?></h1>
                                        ...
                                        foreach ($semesters as $semester) {
                                            ...
                                            if (sizeof($subjectReturn) > 0) {
                                                ...
                                                <form action="/students/<?= $student['studentId'] ?>/update/subjects" method="post"
                                                    class=" max-h-full">
                                                    <input type="text" name="year" id="year" value="<?= $year['subject_year'] ?>"
                                                        hidden>
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        <div
                                                            ...
                                                            <div class="font-semibold"><?= "{$YEARNAME} - {$termDivisor}" ?>
                                                            </div>
                                                            <div class="font-semibold"><?= "A.Y. {$startYear} - {$endYear}" ?></div>
                                                        </div>
                                                            ...
                                                            foreach ($subjectReturn as $subject) {
                                                                ...
                                                                if ($subject["subject_electives"] != "None") {
                                                                    if (!$elec_exist) {
                                                                        ...
                                                                        $code = "";
                                                                        $description = "Elective Subject #{$elec_increment}";
                                                                        $units = "";
                                                                        $grade = "";

                                                                        if (!$student['elective']) {
                                                                            $electives = $db->query(
                                                                                "SELECT * FROM `{$studentTable}` WHERE subject_year = ? AND subject_semester = ? AND subject_electives = ? AND status NOT IN ('CANCELLED', 'INCREMENTED', 'ADVANCED')",
                                                                                [$year['subject_year'], $semester['subject_semester'], $student['elective']]
                                                                            )->fetch();
                                                                            if (!empty($electives)) {
                                                                                $code = $electives['subject_code'];
                                                                                $units = $electives['subject_total_units'];
                                                                                $grade = $electives['grade'];
                                                                                $description = $electives['subject_description'];
                                                                            }
                                                                        }
                                                                        ?>
