...
$origYears = $db->query("SELECT DISTINCT subject_year FROM `{$studentTable}`")->fetchAll();
$origSemesters = $db->query("SELECT DISTINCT subject_semester FROM `{$studentTable}`")->fetchAll();

$curriculum = $db->query("SELECT * FROM `{$studentTable}` WHERE NOT status = 'CANCELLED' AND NOT status = 'INCREMENTED' AND NOT status = 'ADVANCED'")->fetchAll();
...
$curriculumYears = $db->query("SELECT DISTINCT subject_year FROM `{$studentTable}`")->fetchAll();
$curriculumSemesters = $db->query("SELECT DISTINCT subject_semester FROM `{$studentTable}`")->fetchAll();
